...
class QNodes(SIA):
    ...
    def __init__(self, gestor: Manager):
        ...
        self.memoria_funcion_submodular = dict()
    ...
    def aplicar_estrategia(
        ...
        self.tiempos = (
            # arreglo lleno de ceros con tamaño m (para el futuro)
            np.zeros(self.n, dtype=np.int8),
            # arreglo lleno de unos con tamaño n (para el presente)
        ...
        # aqui es donde se ejecuta el algoritmo Q, le paso los vertices del subsistema y me retorna la particion optima
    ...
    def algorithm(self, vertices: list[tuple[int, int]]):
        ...
        """
        omegas_origen = np.array([vertices[0]]) # primer elemento de "vertices", se envuelve en una lista y luego se convierte en un array de numpy
        deltas_origen = np.array(vertices[1:])  # todos los elementos de "vertices" a partir de la posición 1 (excluyendo el primer elemento), se convierte en un array de numpy
           
        vertices_fase = vertices # lista de vértices, esta se actualiza en cada fase, según se actualice el subsistema que se esté analizando
        ...
        # este ciclo son las  "fases", este sería el ciclo externo. Por ejemplo digamos que "vertices" tiene 4 elementos V = {At, Bt, At+1, Bt+1} 
        # el elemento V1 = At. V1 siempre va a ser el primer nodo del subsistema V para este algoritmo.
        
        # la i iterará sobre todos los elementos de V exeptuando el primero pues es el nodo ya tomado en omega al inicio y el final puesto que ya se conocerá cuál será el peor elemento a tomar al momento que queden 2 y se seleccione el mejor)
        # por lo tanto i iterará 2 veces (4-2 = 2)
        
        emd_particion_candidata = INFTY_POS
        i = 0
        while (emd_particion_candidata != 0 and (i <= len(vertices_fase) - 2)):
            #print(f"i: {i}")
            self.logger.debug(f"total: {total-i}")
            omegas_ciclo = [vertices_fase[0]]   # primer elemento de la lista de la V analizada en esta fase
            deltas_ciclo = vertices_fase[1:]    # todos los elementos de la lista de la V analizada en esta fase a partir de la posición 1 (excluyendo el primer elemento)

            # este ciclo es el que actualiza los valores de el conjunto de omegas y deltas
            # es decir, los conjuntos omegas_ciclo y deltas_ciclo van a estar cambiando en el ciclo de k, y lo que hace este ciclo es iterar para averiguar cual es la mejor combinacion de omegas_ciclo y deltas_ciclo que generen la menor EMD
            emd_particion_candidata = INFTY_POS
            
            for j in range(len(deltas_ciclo) - 1):
                #print(f"j: {j}")
                #print(f"j: {j} con valores de W{j+1} = {omegas_ciclo} y Deltas = {deltas_ciclo}")
                # self.logger.critic(f"   {j=}")
                emd_local = 1e5
                indice_mip: int
                # print(f"omegas_ciclo: {omegas_ciclo}")
                # print(f"deltas_ciclo: {deltas_ciclo}")
                # print("\n")
                
                # este es el ciclo interno, se encarga de calcular el EMD de la particion omega (omegas ciclo) multiplicada por la particion delta, para cada delta en deltas_ciclo.
                # esto vendría a ser como: retornar 'k' que devuelva el minimo[g(Wᵢ ∪ {u}) - g({u})] para u ∈ {Bt, At+1, Bt+1} : {(0, 1), (1, 0), (1, 1)}
                # u: conjunto de nodos delta que se están evaluando en el ciclo actual de i
                # Después de hacer esta evaluación para todos los deltas, seleccionar cual brindó una menor EMD al unirse con omega
                for k in range(len(deltas_ciclo)):
 
                    key = (tuple(deltas_ciclo[k]), make_hashable(omegas_ciclo))
                    # -----> print(f"key: {key}")
                    if key in self.memoria_funcion_submodular:
                        emd_union, emd_delta, dist_marginal_delta = self.memoria_funcion_submodular[key]
                        #print(f"Usando valores en caché para {key}")
                    else:
                        emd_union, emd_delta, dist_marginal_delta = self.funcion_submodular(
                            deltas_ciclo[k], omegas_ciclo
                        )
                        self.memoria_funcion_submodular[key] = (emd_union, emd_delta, dist_marginal_delta)
                    
                    # print(f"omegas ciclo: {omegas_ciclo}")
                    # print(f"delta ciclo: {deltas_ciclo[k]}")
                    ...
                    #print(f"emd_particion_candidata iteracion k '{k}': {emd_particion_candidata}")
                    ...
                # self.logger.critic(f"       [k]: {indice_mip}")
                #print(f"indice de iteracion usado (indice mip): {indice_mip}")

                # al final se obtiene el indice del delta que haya obtenido la menor EMD al unirse con omega.
                # Este delta es añadido al conjunto de omegas que se este validando en el ciclo actual de i
                # de igual forma se retira dicho delta de la lista de deltas que se estén validando en el ciclo actual de i
                omegas_ciclo.append(deltas_ciclo[indice_mip])
                deltas_ciclo.pop(indice_mip)
                ...
            i += 1
            # por acá se calcula si el emd_delta es igual a 0, si es así, se rompe el ciclo de i, puesto que ya se ecnontro la particion candidata mas grande
            #print(f"\n --> EMD de la particion candidata escogida en el ciclo i '{i}': {emd_particion_candidata}\n\n")
            ...
            ...
        # se retorna la partición con menor EMD
        return min(
    ...
    def funcion_submodular(
        ...
        """
        emd_delta = INFTY_NEG
        # temporal es una lista de dos listas que guarda los indices de los delta en el presente [0] y en el futuro [1]
        # por ejemplo: temporal = [[3], [2, 5]] indica que el nodo 3 está en el presente y los nodos 2 y 5 están en el futuro
        temporal = [[], []] # temporal[0] = indices de los nodos en el presente
                            # temporal[1] = indices de los nodos en el futuro

        # print(f"omegas que me llegan: {omegas}")
        # print(f"deltas que me llegan: {deltas}")
        
        if isinstance(deltas, tuple): # si 'deltas' es una sola tupla ***es decir, un solo nodo (tiempo, indice)*** guarda este tiempo e indice en la lista correspondiente
            d_tiempo, d_indice = deltas
            temporal[d_tiempo].append(d_indice) # ej: si deltas es (0, 3), guardará en t[0] el indice 3

        else: # si 'deltas' es una lista de tuplas ***es decir, un grupo de nodos (tiempo, indice), (tiempo, indice), ...*** guarda estos tiempos e indices en la lista correspondiente
            for delta in deltas:
                d_tiempo, d_indice = delta
                temporal[d_tiempo].append(d_indice)

        # print(f"temporal: {temporal}")
        # Esta es una copia del subsistema original V (que resulta de la preparación con la funcion sia_preparar_subsistema), se hace para no modificar el subsistema original
        copia_delta = self.sia_subsistema

        dims_mecanismo_delta = temporal[ACTUAL] # temporal[0], osea, indices de los deltas en el presente
        dims_alcance_delta = temporal[EFECTO]   # temporal[1], osea, indices de los deltas en el futuro
        ...
        # distribucion marginal: distribucion de probabilidad de un subsistema o particion en especifico
                            # distribucion marginal de delta, distribucion marginal del subsistema completo
        # print(f"vertices de delta : {particion_delta}")
        # print(f"vector_delta_marginal: {vector_delta_marginal}")
        # print(f"sia_dists_marginales: {self.sia_dists_marginales}\n\n")
