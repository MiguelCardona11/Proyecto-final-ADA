...
class System:
    ...
    def bipartir(
        ...
        """
        # print(f"Alcance que llega a bipartir: {alcance}")
        # print(f"Mecanismo que llega a bipartir: {mecanismo}")
        new_sys = System.__new__(System)
        new_sys.estado_inicial = self.estado_inicial

        new_sys.ncubos = tuple(
            # Si cube.indice está en alcance, marginaliza los elementos de cube.dims que no están en mecanismo
            # Si cube.indice NO está en alcance, marginaliza todos los indices que estén en mecanismo
            
            #! paralelizar el proceso de marginalización
            cube.marginalizar(np.setdiff1d(cube.dims, mecanismo)) # elimina las dimensiones que no están en los indices "mecanismo" que me llegaron
            if cube.indice in alcance
            else cube.marginalizar(mecanismo)
            for cube in self.ncubos 
        )
        # print(f"{new_sys.ncubos}\n")
        # Se almacena el resultado como una tupla de NCube en "new_sys", representando los cubos transformados, retorna este nuevo sistema
        return new_sys
    ...
    def distribucion_marginal(self):
        ...
        """
        probabilidad: float
        # indices.ncubos: si el subsistema es A, B, C entonces indices_ncubos = [0, 1, 2]
        # crea un arreglo vacio de tamaño igual a la cantidad de elementos en indices_ncubos
